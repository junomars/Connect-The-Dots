...
class Point(GraphicsObject):
    ...
    def distanceTo(self, point): return math.sqrt(math.pow(self.x-point.getX(), 2) + math.pow(self.y-point.getY(), 2))
...
class _BBox(GraphicsObject):
    ...
    def isPointInsideMe(self, point):
        return self.p1.x <= point.getX() <= self.p2.x and self.p1.y <= point.getY() <= self.p2.y
...
class Oval(_BBox):
    ...
    def isPointInsideMe(self, point):
        radiusX = (self.p2.x - self.p1.x) / 2
        radiusY = (self.p2.y - self.p1.y) / 2
        coordX = point.getX()-self.getCenter().x
        coordY = point.getY()-self.getCenter().y
        return (math.pow(coordX, 2) / math.pow(radiusX, 2) + math.pow(coordY, 2) / math.pow(radiusY, 2)) <= 1

    def slopeAt(self, point):
        radiusX = (self.p2.x - self.p1.x) / 2
        radiusY = (self.p2.y - self.p1.y) / 2
        if (radiusX * radiusX * (point.getY() - self.getCenter().getY() == 0)):
            return -1 * radiusY * radiusY * (point.getX() - self.getCenter().getX()) / .0000000000005
        else:
            return (-1 * radiusY * radiusY * (point.getX() - self.getCenter().getX()) /
                (radiusX * radiusX * (point.getY() - self.getCenter().getY())))
